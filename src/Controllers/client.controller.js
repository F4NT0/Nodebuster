...
exports.create = (req,res) => {
    ...
    Client.create(client,(err,data) => {
        if(err){
            res.status(500).send({
                message: err.message || "Error on client creation!"
            });
        };
        res.send(data);
    });
...
exports.findAll = (req,res) => {
    Client.getAll((err,data) => {
        if(err){
            res.status(500).send({
                message: err.message || "Error on getting all clients!"
            });
        };
        res.send(data);
    });
    ...
    Client.findById(req.params.id, (err,data) => {
        if(err){
            if(err.kind === 'not_found'){
                res.status(404).send({
                    message: `No client with id ${req.params.id}`
                });
            }else{
                res.status(505).send({
...
exports.update = (req,res) => {
    ...
    Client.updateById(req.params.id_client,new Client(req.body),(err,data) => {
        if(err){
            if(err.kind === 'not_found'){
                res.status(404).send({
                    message: `No client with title ${req.params.id_client}`
                });
            }else{
                res.status(505).send({
                    message: `Error updating client with ID ${req.params.id_client}`
                });
            }
        }else{ 
            res.send(data);
...
exports.delete = (req,res) => {
    Client.remove(req.params.id,(err,data) => {
        if(err){
            ...
            message: `Client was deleted Sucessfully!`
        });
    });
};